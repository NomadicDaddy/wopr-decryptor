...
var WOPRDecryptor = class {
  ...
  mergeOptions(o) {
    ...
    if (timing.endDateTime) {
      ...
      const startDate = timing.startDateTime ? timing.startDateTime instanceof Date ? timing.startDateTime : new Date(timing.startDateTime) : /* @__PURE__ */ new Date();
  ...
  prepareCode(finalCode) {
    ...
    const dynamicIdx = t.map((ch, i) => ({ ch, i })).filter((o) => /[A-Z0-9]/.test(o.ch)).map((o) => o.i);
    this.template = t;
  ...
  calculateDigitsToSolve() {
    ...
    const startDate = this.opts.timing.startDateTime instanceof Date ? this.opts.timing.startDateTime : new Date(this.opts.timing.startDateTime);
    const endDate = this.opts.timing.endDateTime instanceof Date ? this.opts.timing.endDateTime : new Date(this.opts.timing.endDateTime);
